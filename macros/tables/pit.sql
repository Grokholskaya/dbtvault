...
new_rows AS (
    ...
    {%- for sat_name in satellites -%}
        ...
        MAX({{ dbtvault.escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_pk }}) AS {{ dbtvault.escape_column_names( sat_name  ~ '_' ~ sat_pk_name | lower) }},
        {%- else -%}
        ...
        MAX({{ dbtvault.escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_ldts }})  AS {{ dbtvault.escape_column_names( sat_name  ~ '_' ~ sat_ldts_name | lower ) }}
        {%- else -%}
        MAX({{ dbtvault.escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_ldts }})  AS {{ dbtvault.escape_column_names( sat_name  ~ '_' ~ sat_ldts_name ) }}
        {%- endif -%}
        {{- "," if not loop.last }}
    {%- endfor %}
